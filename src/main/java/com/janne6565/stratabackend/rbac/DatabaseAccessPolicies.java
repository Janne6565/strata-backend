...
public class DatabaseAccessPolicies {
    ...
    public boolean canBrowse(ResourceResolver resolver, Object referenceId, User caller) {
        return canRead(resolver, referenceId, caller);
    }

    @Validates(Operation.DB_QUERY_READ)
    public boolean canReadQuery(ResourceResolver resolver, Object referenceId, User caller) {
        return canRead(resolver, referenceId, caller);
    }

    @Validates(Operation.DB_QUERY_WRITE)
    public boolean canWriteQuery(ResourceResolver resolver, Object referenceId, User caller) {
        Datasource datasource = resolver.requireDatasource(referenceId);
        return grantEvaluator.canWrite(caller, datasource);
    }

    private boolean canRead(ResourceResolver resolver, Object referenceId, User caller) {
        Datasource datasource = resolver.requireDatasource(referenceId);
